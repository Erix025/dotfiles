...
if ! command -v zsh &>/dev/null; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        echo "Debian/Ubuntu: 使用 apt 安装 zsh"
        sudo apt update
        sudo apt install -y zsh
    elif [[ -f /etc/redhat-release ]]; then
        echo "RHEL/CentOS/Fedora: 使用 dnf/yum 安装 zsh"
        sudo dnf install -y zsh || sudo yum install -y zsh
    else
        echo "⚠️ 未知系统，请手动安装 zsh"
        exit 1
    fi
else
    echo "✅ zsh 已安装"
...
echo "=== Step 3: 启动 Bitwarden SSH Agent ==="
