...
-antigen-ensure-repo () {
  ...
  if [[ ! -d $clone_dir ]]; then
    ...
    echo "Updating submodules... for $clone_dir"
    if [[ -f "${clone_dir}/.gitmodules" ]]; then
      # Sync the changes to .git/config
      --plugin-git submodule sync
    fi
...
-antigen-env-setup () {
  ...
  -antigen-set-default ANTIGEN_CLONE_OPTS "--single-branch --depth=1"
  -antigen-set-default ANTIGEN_SUBMODULE_OPTS "--depth=1"
